...
exports.matchRoutes = function (req, res) {
  ...
  var section = urlParts[2] ? urlParts[1] : urlParts[0]
  var page = urlParts[2] ? urlParts[2] : urlParts[1]
  ...
  res.render(path, {
    ...
    if (err) {
      // TODO make this a bit nicer
      var indexPath = path.replace(/\/$/, '') + '/index'
      var urlParts = indexPath.split('/').splice(1)
      var indexSection = urlParts[0] && urlParts[0]
      var indexPage = urlParts[0] + '-index'
